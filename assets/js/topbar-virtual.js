$(function() {
  ...
  const aboutSection = document.getElementById('about'); // The ID of the section to scroll to
  
  if (scrollDownButton && aboutSection) {
    scrollDownButton.addEventListener('click', () => {
      scrollDownButton.classList.add('shake'); // Add the shake class
      aboutSection.scrollIntoView({ behavior: 'smooth' }); // Smooth scroll

      // Remove the shake class after the animation completes
      setTimeout(() => {
        scrollDownButton.classList.remove('shake');
      }, 300); // 300ms = 0.3s (same as the animation duration)
    });
  }
...
//Add this to the topbar-virtual.js file
